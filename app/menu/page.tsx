        ...
        <div className="scroll-mt-24" id={title.toLowerCase().replace(/\s+/g, '-')}>
            ...
            <div className="grid md:grid-cols-2 gap-x-16 gap-y-16">
                {categories.map((category, idx) => (
                    <div key={idx} className={`${isBuffet ? 'bg-secondary/20 p-8 rounded-2xl border border-primary/5' : ''}`}>
                        <div className="flex items-center gap-4 mb-8">
                            <h3 className="text-xl font-serif text-primary italic whitespace-nowrap">{category.category}</h3>
                            ...
                            {category.items.map((item, itemIdx) => (
                                <div key={itemIdx} className="group">
                                    <div className="flex justify-between items-baseline mb-1">
                                        <h4 className="font-serif group-hover:text-primary transition-colors text-lg">
                                            {item.name}
                                            {item.isVegetarian && <span className="text-green-500 text-xs ml-2" title="Vegetarian">(V)</span>}
                                        </h4>

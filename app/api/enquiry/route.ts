...
export async function POST(request: Request) {
    try {
        ...
        const { fullName, phone, email, enquiryType, guests, preferredDate, message } = body;

        // 1. Validation
        if (!fullName || !phone || !email || !enquiryType) {
            return NextResponse.json(
                { error: 'Missing required fields: fullName, phone, email, and enquiryType are required.' },
                { status: 400 }
            );
        }
        ...
        const customerMailOptions = {
            ...
            html: `
                <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; padding: 40px; border: 1px solid #C5A059; border-radius: 20px; background-color: #0B1C2C; color: #F5F5F5;">
                    ...
                    <p style="font-size: 16px; line-height: 1.6; color: #FFFFFF;">Hi ${fullName},</p>
                    <p style="font-size: 16px; line-height: 1.6; color: #FFFFFF;">Thank you for reaching out to us. We have received your enquiry and a member of our team will be in touch with you shortly.</p>
                    
                    <h2 style="color: #C5A059; font-size: 18px; text-transform: uppercase; letter-spacing: 1px; margin-top: 30px; border-bottom: 1px solid rgba(197,160,89,0.1); padding-bottom: 10px;">Enquiry Summary</h2>

                    <div style="margin-top: 20px; display: grid; gap: 15px;">
                        <div>
                            <p style="text-transform: uppercase; font-size: 10px; letter-spacing: 2px; color: #C5A059; margin-bottom: 2px;">Type</p>
                            <p style="font-size: 14px; margin: 0; color: #FFFFFF;">${enquiryType}</p>
                        </div>
                        ${preferredDate ? `
                        <div>
                            <p style="text-transform: uppercase; font-size: 10px; letter-spacing: 2px; color: #C5A059; margin-bottom: 2px;">Preferred Date</p>
                            <p style="font-size: 14px; margin: 0; color: #FFFFFF;">${preferredDate}</p>
                        </div>` : ''}
                        ${guests ? `
                        <div>
                            <p style="text-transform: uppercase; font-size: 10px; letter-spacing: 2px; color: #C5A059; margin-bottom: 2px;">Expected Guests</p>
                            <p style="font-size: 14px; margin: 0; color: #FFFFFF;">${guests}</p>
                        </div>` : ''}
                    </div>

                    <div style="margin-top: 40px; padding-top: 20px; border-top: 1px solid rgba(197,160,89,0.2); text-align: center;">
                        <p style="font-size: 16px; line-height: 1.6; color: #FFFFFF; font-weight: bold;">We look forward to serving you.</p>
